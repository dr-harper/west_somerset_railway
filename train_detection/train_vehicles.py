"""Fine-tune YOLO to see the vehicles a train is made of.
...
DATA = HERE / 'dataset_vehicles' / 'data.yaml'
RUNS = HERE / 'runs'
BASE = 'yolo11s.pt'
...
def train(epochs: int, imgsz: int, batch: int, device: str, patience: int) -> None:
    ...
    model.train(
        ...
        project=str(RUNS), name='vehicles', exist_ok=True,
...
def compare(device: str) -> None:
    ...
    trained = RUNS / 'vehicles' / 'weights' / 'best.pt'
